...
export function TableStatus() {
    ...
    useEffect(() => { console.log('[DEBUG] TableStatus received:', tableStatuses); }, [tableStatuses]);

    const getStatusStyles = (status: TableStatusType['status']) => {
        switch (status) {
            case 'Empty': return 'bg-green-50 border-green-200 text-green-800';
    ...
    const handleClear = (e: React.MouseEvent, tableId: number) => {
        e.stopPropagation();
        if (confirm('Clear table? This will start a new session for the next customer.')) {
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>Live overview. Click "Clear" when customers leave to reset the table.</CardDescription>
            </CardHeader>
            <CardContent>
                        ...
                        {tableStatuses.sort((a, b) => {
                            ...
                            >
                                <div className="flex flex-col items-center gap-1 mt-2">
                                    ...
                                    <Badge variant="outline" className="bg-black/70 border-0 text-[12px] h-7 mt-2">{table.status}</Badge>
                                </div>
