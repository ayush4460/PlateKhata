...
export default function OrdersPage() {
  ...
  const tableSessionOrders = useMemo(() => {
    ...
    pastOrders.forEach((order, index) => {
      ...
      if (String(order.tableNumber) !== String(tableNumber)) {
        //console.log(`[ORDERS PAGE DEBUG] Order ${order.id} - table mismatch, skipping`);
        return;
      }
      ...
      const isCompleted = statusLower === 'completed' && order.paymentStatus === 'Approved';
    ...
    return {
      active: active.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()),
      completed: completed.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())
    };
  }, [pastOrders, tableNumber, currentTime]);

  const allDisplayedOrders = [...tableSessionOrders.active, ...tableSessionOrders.completed];

  // Calculate Grand Totals
  const sessionTotals = useMemo(() => {
    return allDisplayedOrders.reduce((acc, order) => ({
  ...
  const isPaymentPending = tableSessionOrders.active.some(o => o.paymentStatus === 'Pending');
  const isFullyPaid = allDisplayedOrders.length > 0 && allDisplayedOrders.every(o => o.paymentStatus === 'Approved');
...
const handleDownloadCombinedReceipt = () => {
  ...
  doc.text('Where Every Meal is a Celebration', pageWidth / 2, 35, { align: 'center' });
  ...
  doc.roundedRect(pageWidth - 95, summaryBoxY, 81, summaryBoxHeight, 3, 3, 'S');
  ...
  doc.setTextColor(34, 197, 94); // Green
  doc.setFontSize(14);
  doc.text(formatCurrency(sessionTotals.total), pageWidth - 18, yPos, { align: 'right' });
  ...
  if (isFullyPaid) {
    ...
    doc.text('PAID IN FULL', pageWidth - 44.5, badgeY + 6, { align: 'center' });
  }
  ...
  doc.setFont('helvetica', 'italic');
  doc.setTextColor(134, 25, 143); // Deep pink
  doc.text('We hope to serve you again soon', pageWidth / 2, footerY + 17, { align: 'center' });
  ...
  doc.text('This is a computer generated invoice. No signature required.', pageWidth / 2, footerY + 31, { align: 'center' });
  ...
  return (
    <div className="bg-background min-h-screen pb-32">
      ...
      {tableNumber && tableSessionOrders.active.length > 0 && isPaymentPending && (
