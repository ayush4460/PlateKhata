...
interface AuthContextType {
  ...
  adminLogin: (user: AdminUser, accessToken: string, refreshToken?: string) => Promise<boolean>;
  logout: () => void;
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (token && storedAdminUser) {
        console.log('[AuthContext] Found existing session for:', storedAdminUser.email);
  ...
  const adminLogin = useCallback(async (user: AdminUser, accessToken: string, refreshToken?: string): Promise<boolean> => {
    try {
      ...
      console.error('[AuthContext] Failed to save session to localStorage:', error);
